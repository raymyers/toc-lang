...
export default function Tree({ ast }) {
  console.log("rendering tree with ast: ", ast);
    ...
    ast.statements.filter(s => s.type === 'status').forEach(statement => {
    ...
    ast.statements.filter(s => s.type === 'CSF').forEach(statement => {
        createNode({key: statement.id, label: statement.text, primary: true});
        createEdge({from: 'goal', to: statement.id});
    });
    ast.statements.filter(s => s.type === 'NC').forEach(statement => {
        createNode({
          key: statement.id, 
    ...
    ast.statements.filter(s => s.type === 'requirement').forEach(statement => {
        const nodeKey = statement.id;
