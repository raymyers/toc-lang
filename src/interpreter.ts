...
const grammarPromise = fetch(peggyGrammarUrl).then((response) => response.text())
let parserPromise = grammarPromise.then((peggyGrammar) => { return peggy.generate(peggyGrammar)});
...
const checkSemantics = (ast) => {
    ...
    ast.statements.filter(statement => statement.type == "NC" || statement.type == "CSF").forEach((statement) => {
    ...
    ast.statements.filter(statement => statement.type == "requirement").forEach((statement) => {
        ...
        statement.requirements.forEach((reqId) => {
            ...
            if (csfNodeIds.has(reqId)) {
                throw new Error(`${nodeId} cannot require ${reqId}, because CSFs are only required by the Goal`);
            }
